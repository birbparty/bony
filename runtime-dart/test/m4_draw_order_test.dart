// Dart M4 numeric gate: multi-bone transform chain vs committed M4 golden.
//
// Loads m4_rig.bony (3-bone chain: root → upper_arm → lower_arm), computes
// setup-pose world transforms and draw batches, and compares numeric values
// against the committed golden (conformance/goldens/m4_rig_t0.json)
// within the Dart tolerance (abs <= 1e-4).
//
...
void main() {
  ...
  setUpAll(() {
    data = loadBonyJson(
      File('../conformance/assets/m4_rig.bony').readAsStringSync(),
    );
    worlds = computeWorldTransforms(data);
    batches = buildDrawBatches(data);
    golden = jsonDecode(
      File('../conformance/goldens/m4_rig_t0.json').readAsStringSync(),
    ) as Map<String, dynamic>;
  });

  group('M4 golden format metadata', () {
  ...
  group('M4 world transforms (abs <= 1e-4)', () {
    test('all 3 bones are computed', () {
    ...
    test('multi-level inherited transform (lower_arm) is correct', () {
      // lower_arm inherits from upper_arm which inherits from root.
      // This is the key test of parent-chain composition.
      final goldenBones = (golden['bones'] as List<dynamic>)
          .cast<Map<String, dynamic>>();
      final lowerArmGolden = goldenBones.firstWhere(
        (b) => (b['name'] as String) == 'lower_arm',
      );
      final lowerArmIdx = data.bones.indexWhere((b) => b.name == 'lower_arm');
      _expectAffine(
        worlds[lowerArmIdx],
        lowerArmGolden['world'] as Map<String, dynamic>,
        'lower_arm.world',
      );
    });
  });

  group('M4 draw batches (4-slot rig)', () {
  ...
  group('M4 rig has no animations (static rig)', () {
    test('animations list is empty', () {
