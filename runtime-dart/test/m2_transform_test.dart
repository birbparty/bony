// Dart M2 numeric gate: world transforms + draw batches vs committed M2 golden.
//
// Loads m2_rig.bony, computes the setup-pose world transforms and draw batches,
// and compares numeric values against the committed golden
// (conformance/goldens/m2_rig_t0.json) within the Dart tolerance (abs <= 1e-4).
//
...
void main() {
  ...
  setUpAll(() {
    ...
    golden = jsonDecode(
      File('../conformance/goldens/m2_rig_t0.json').readAsStringSync(),
    ) as Map<String, dynamic>;
  });

  group('M2 golden format metadata', () {
  ...
  group('M2 world transforms (abs <= 1e-4)', () {
    test('all 7 bones are computed', () {
  ...
  group('M2 all other rigs pass through Dart loader + transform', () {
    test('m1 rig world transforms compute without error', () {
      final d = loadBonyJson(
        File('../conformance/assets/m1_rig.bony').readAsStringSync(),
      );
