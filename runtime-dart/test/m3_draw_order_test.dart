// Dart M3 numeric gate: multi-slot draw ordering vs committed M3 golden.
//
// Loads m3_rig.bony, computes setup-pose world transforms and draw batches,
// and compares numeric values against the committed golden
// (conformance/goldens/m3_rig_t0.json) within the Dart tolerance (abs <= 1e-4).
//
...
void main() {
  ...
  setUpAll(() {
    ...
    golden = jsonDecode(
      File('../conformance/goldens/m3_rig_t0.json').readAsStringSync(),
    ) as Map<String, dynamic>;
  });

  group('M3 golden format metadata', () {
  ...
  group('M3 world transforms (abs <= 1e-4)', () {
    test('all 6 bones are computed', () {
  ...
  group('M3 draw batches (multi-slot draw ordering)', () {
    ...
    test('draw order matches slot declaration order in rig', () {
      // M3 tests that draw batch order == slot declaration order
  ...
  group('M3 rig has no animations (static rig)', () {
