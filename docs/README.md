...
| [skin-required-activation-contract.md](skin-required-activation-contract.md) | Active-skin membership and runtime/load semantics for `skinRequired` bones plus IK, transform, path, and physics constraints |
| [nested-rig-attachment-contract.md](nested-rig-attachment-contract.md) | Slot-visible nested rig attachment records: top-level `nestedRigAttachments[]`, JSON/BNB shape, slot/skin target validation, canonical ordering, and deferred runtime playback/asset-loading boundaries |
| [nested-rig-runtime-composition-contract.md](nested-rig-runtime-composition-contract.md) | Host-resolved nested rig setup-pose composition: parent affine, child active skin, draw-order insertion, clipping interaction, recursion/cycle handling, and missing-child failures |
| [draw-order-timeline-contract.md](draw-order-timeline-contract.md) | Clip-owned draw-order timeline that restacks setup slots over time: JSON shape, offset permutation validation, packed `drawOrderKeys` byte layout, stepped sampling, clipping invariants, and non-goals |
